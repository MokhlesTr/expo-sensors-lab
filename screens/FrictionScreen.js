...
import {
  ...
  TouchableOpacity, Image, Platform, Animated,
} from 'react-native';
...
// Each card: initial rotation offset + friction multiplier
// friction=0 → stays still, friction=2.5 → moves most
const CARDS = [
  { initRot: -18, friction: 2.5, scale: 0.80, zIndex: 1 },
  ...
  { initRot:  0,  friction: 0.0, scale: 1.00, zIndex: 5 }, // front card — stationary
];

export default function FrictionScreen({ navigation }) {
  // Single gyro position: left/right (X) and forward/back (Y)
  const gyroX = useRef(new Animated.Value(0)).current;
  const gyroY = useRef(new Animated.Value(0)).current;

  // Accumulated + decayed values (prevents drift to edges)
  const accX = useRef(0);
  const accY = useRef(0);
  ...
  useEffect(() => {
    ...
    Gyroscope.setUpdateInterval(16); // 60 Hz for buttery smooth parallax

    const sub = Gyroscope.addListener(({ x, y }) => {
      // Low-pass decay — cards return to resting position when phone is still
      accX.current = accX.current * 0.80 + y * 3.0;
      accY.current = accY.current * 0.80 + x * 3.0;

      // setValue is O(1) — zero allocation, no springs created here
  ...
  const entranceTranslate = entrance.interpolate({ inputRange: [0, 1], outputRange: [80, 0] });
    ...
    <View style={styles.root}>
      ...
      >
        {PHOTOS.map((uri, i) => {
          ...
          // Each card shifts by (gyroX × friction) — different per card = friction parallax
          ...
          // The rotation also fans out based on gyro + initial offset
          ...
          // Staggered entrance: back cards come from further down
          ...
          return (
            ...
            >
              ...
              {/* Label on front card only */}
              {i === PHOTOS.length - 1 && (
                <View style={styles.frontLabel}>
                  <View style={styles.frictionBadge}>
                    <Text style={styles.frictionBadgeTxt}>🌀  Tilt to fan cards</Text>
                  </View>
                </View>
              )}
            </Animated.View>
...
const styles = StyleSheet.create({
  ...
  topBar:      { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: Platform.OS === 'ios' ? 56 : 36, marginBottom: 8 },
  ...
  appName:  { fontSize: 30, fontWeight: '800', color: '#111', letterSpacing: -0.5, marginBottom: 6 },
  tagline:  { fontSize: 14, color: '#666', lineHeight: 20, marginBottom: 18 },
  ctaBtn:   { backgroundColor: '#111', borderRadius: 50, paddingVertical: 16, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 10, elevation: 6 },
