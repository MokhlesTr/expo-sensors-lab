...
type Props = NativeStackScreenProps<RootStackParamList, "Gyroscope">;
...
export default function GyroscopeScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    ...
    const sub = Gyroscope.addListener((g) => {
      ...
      accZ.current = accZ.current * 0.8 + g.z * 40;
      ...
      if (frame.current % 4 === 0) {
        const moving =
          Math.abs(g.x) > 0.05 || Math.abs(g.y) > 0.05 || Math.abs(g.z) > 0.05;
    ...
    <View style={styles.root}>
      ...
      >
        ...
        <Text style={styles.hint}>
          ↔ Tilt device · each ring has different friction
        </Text>
        ...
        <View style={styles.cards}>
          {[
            ...
            >
              ...
              <Text style={[styles.cardSub, { color: color + "AA" }]}>
                {sub}
              </Text>
            </View>
          ))}
...
const styles = StyleSheet.create({
  ...
  ringInner: {
    width: RING_SIZE * 0.52,
    height: RING_SIZE * 0.52,
    borderWidth: 2.5,
    borderColor: "#34C759",
  },
  centerDot: {
