...
type Props = NativeStackScreenProps<RootStackParamList, "Imaze">;
...
const PHOTOS: Photo[] = [
  {
    id: "1",
    uri: "https://images.unsplash.com/photo-1499678329028-101435549a4e?w=800&q=85",
    label: "Cinque Terre",
    sub: "Italy",
  },
  {
    id: "2",
    uri: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800&q=85",
    label: "Swiss Alps",
    sub: "Switzerland",
  },
  {
    id: "3",
    uri: "https://images.unsplash.com/photo-1516483638261-f4dbaf036963?w=800&q=85",
    label: "Santorini",
    sub: "Greece",
  },
  {
    id: "4",
    uri: "https://images.unsplash.com/photo-1501854140801-50d01698950b?w=800&q=85",
    label: "Fjords",
    sub: "Norway",
  },
  {
    id: "5",
    uri: "https://images.unsplash.com/photo-1477959858617-67f85cf4f1df?w=800&q=85",
    label: "City Lights",
    sub: "New York",
  },
];
...
export default function ImazeScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    ...
    const sub = Gyroscope.addListener(({ x, y }) => {
      ...
      const cx = Math.max(
        -PARALLAX_EXTRA,
        Math.min(PARALLAX_EXTRA, accX.current),
      );
      const cy = Math.max(
        -PARALLAX_EXTRA,
        Math.min(PARALLAX_EXTRA, accY.current),
      );
      shiftX.setValue(cx);
      shiftY.setValue(cy);
    });
  ...
  const onViewableItemsChanged = useCallback(({ viewableItems }: any) => {
    if (viewableItems.length > 0)
      setActiveIdx(Number(viewableItems[0].key) - 1);
  }, []);
        ...
        renderItem={({ item, index }) => (
          <TouchableOpacity onPress={() => goTo(index)} activeOpacity={0.8}>
            <View
              style={[
                styles.thumbWrap,
                index === activeIdx && styles.thumbActive,
              ]}
            >
              <Image
                source={{ uri: item.uri }}
